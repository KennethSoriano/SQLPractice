...
SELECT customer_id, COUNT(order_id) AS order_count FROM order
...
FROM territories t JOIN regions r ON t.region_id = r.region_id
WHERE t.territory_id NOT IN (
    SELECT e.territory_id FROM employees_territories e
)
